class RequestDetailsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @request_detail.quanity.to_i > @book.quanity.to_i
  ...
  def update
    ...
    @book = @request_detail.book
    @book_quanity = @request_detail.quanity + @request_detail.book.quanity
    @request_detail.attributes = update_params
    respond_to do |format|
      if @request_detail.quanity.to_i < @book_quanity.to_i
        @request_detail.save
        @book.quanity = @book_quanity - @request_detail.quanity
        @book.update_attributes(quanity: @book.quanity)
        format.html { redirect_to cart_path(current_cart), notice: "Successfully updated." }
      else
        format.html { redirect_to cart_path(current_cart), alert: "Unsuccess update." }
      end
    end
  ...
  def update_params
    params.require(:request_detail).permit(:quanity, :book_id)
  end
end
